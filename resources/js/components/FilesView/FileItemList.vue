...
export default {
	...
	computed: {
		...mapGetters(['FilePreviewType', 'permissionOptions']),
		isFolder() {
	...
	methods: {
		...
		goToItem() {
            ...
            if(this.isMdFile && this.$isSharedRoute()){
                events.$emit('fileFullPreview:show')
            }

            if (this.isMdFile && !this.isDeleted && !this.$isSharedRoute()) {
