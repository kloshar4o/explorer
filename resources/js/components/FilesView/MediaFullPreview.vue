    ...
    <div class="media-full-preview" id="mediaPreview" v-if="this.isMedia && fileInfoDetail">
        <div class="file-wrapper-preview" v-for="i in [currentIndex]" :key="i">
            <div class="file-wrapper">
                <audio v-if="fileInfoDetail.type == 'audio'"
                       class="file audio"
                ...
                <img v-if="fileInfoDetail.type === 'image' && currentFile.thumbnail"
                     class="file"
                     :class="{ 'file-shadow': !isMobileDevice }"
                     id="image"
                     :src="currentFile.file_url"/>
                <div v-if="fileInfoDetail.type === 'video' && currentFile.file_url" class="video-wrapper">
                    <video :src="currentFile.file_url" class="video" :class="{ 'file-shadow': !isMobileDevice }"
                           controlsList="nodownload" disablePictureInPicture playsinline controls/>
                </div>
                <MdFilePreview v-if="isMdFile" :basename="fileInfoDetail.basename"></MdFilePreview>
            </div>
...
<script>
    ...
    import MdFilePreview from '@/components/FilesView/MdFilePreview'
    import Spinner from '@/components/FilesView/Spinner'

    export default {
        name: 'MediaFullPreview',
        components: {ToolbarButton, Spinner, MdFilePreview},
        computed: {
