...
const actions = {
    ...
    openEditor: ({commit, getters}) => {
        ...
        commit('STORE_PREVIOUS_FOLDER', getters.currentFolder)
        axios
            .get('/api/file/' + getters.fileInfoDetail.basename)
            .then(response => {
                commit('LOADING_STATE', {loading: false, data: []})
                commit('SHOW_EDITOR', true);
                commit('EDITOR_CONTENT', response.data)
            })
            .catch(() => isSomethingWrong())
    }
}
...
const mutations = {
    ...
    ADD_NEW_MD_FILE(state, mdFile) {
        state.data.unshift(mdFile)
    },
    SHOW_EDITOR(state, show) {
        state.showEditor = show;
    },
    EDITOR_CONTENT(state, content) {
