...
class Editor
{
     ...
     */
    public static function save_file($request, $shared = null)
    {

        $file = FileManagerFile::find($request->id);

        if(!$file)
            return response()->json(['success' => 0, 'message' => "Sorry, this file does not exist anymore"]);

        $disk_file_name = $file->basename;
        ...
        $disk_local->put("chunks/$disk_file_name", $markdown);
        $file_size =  $disk_local->size("chunks/$disk_file_name");

        // Check if user has enough space to upload file
        self::check_user_storage_capacity($user_id, $file_size, $disk_file_name);

        if($disk_local->exists("file-manager/$disk_file_name"))
            Storage::delete("file-manager/$disk_file_name");

        $disk_local->move("chunks/$disk_file_name", "file-manager/$disk_file_name");

        // Move files to external storage
        if (!is_storage_driver(['local'])) {

            // Move file to external storage service
            self::move_to_external_storage($disk_file_name, null);
        }

        $file->filesize = $file_size;
        $file->update();

        return response()->json(['success' => 1, 'message' => "$disk_file_name saved successfully"]);
     ...
     * @return FileManagerFolder|\Illuminate\Database\Eloquent\Model
     */
    public static function create_md_file($request, $shared = null)
    {
        $mimetype = "md";
        $type = "file";
        $file_size = 0;
        ...
        // Get variables
        $user_scope = is_null($shared) ? $request->user()->token()->scopes[0] : 'editor';
        $name = $request->has('name') ? $request->input('name') : 'readme';
        $user_id = is_null($shared) ? Auth::id() : $shared->user_id;
        $unique_id = get_unique_id();
        $disk_file_name = "$unique_id-" . time() . "-$name.$mimetype";
        $disk_local = Storage::disk('local');

        $disk_local->put("chunks/$disk_file_name", '');
    ...
    public static function upload($request, $shared = null)
    {
        ...
        $extension = $ext = pathinfo(str_replace('.part', '', $temp_filename), PATHINFO_EXTENSION);;
